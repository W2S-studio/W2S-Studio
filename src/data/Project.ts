export const projects = [
    {
        id: 1,
        key: 'e-commerce',
        title: 'E-Commerce Platform',
        description: 'Modern online store with seamless checkout experience and inventory management',
        imageLink: 'src/assets/image.jpg',
        tags: ['React', 'Node.js', 'Stripe'],
        overlayTitle: 'ShopFlow Pro',
        overlayDescription:
            'A complete e-commerce solution built for a growing fashion brand. Features include real-time inventory tracking, advanced analytics, and AI-powered product recommendations.',
        stats: [
            {
                text: 'Completed in 8 weeks',
                iconPath: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z',
            },
            {
                text: '50K+ monthly users',
                iconPath:
                    'M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3z',
            },
            {
                text: '98.5% uptime',
                iconPath:
                    'M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zM14 9a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1v-6a1 1 0 00-1-1h-2z',
            },
        ],
    },
    {
        id: 2,
        key: 'portfolio',
        title: 'Personal Portfolio',
        description: 'Stylish portfolio site showcasing a designer’s work with interactive elements',
        imageLink: 'src/assets/image.jpg',
        tags: ['Vue.js', 'Tailwind CSS', 'GSAP'],
        overlayTitle: 'Creative Canvas',
        overlayDescription:
            'A dynamic portfolio designed to highlight a designer’s creativity with smooth animations and responsive layouts.',
        stats: [
            {
                text: 'Completed in 4 weeks',
                iconPath: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z',
            },
            {
                text: '10K+ visits',
                iconPath:
                    'M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3z',
            },
            {
                text: '99% mobile compatibility',
                iconPath:
                    'M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zM14 9a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1v-6a1 1 0 00-1-1h-2z',
            },
        ],
    },
    {
        id: 3,
        key: 'blog-platform',
        title: 'Blogging Platform',
        description: 'Robust blog system with user authentication and content management',
        imageLink: 'src/assets/image.jpg',
        tags: ['Next.js', 'MongoDB', 'Prisma'],
        overlayTitle: 'WriteSphere',
        overlayDescription:
            'A feature-rich blogging platform enabling users to create, manage, and monetize content with ease.',
        stats: [
            {
                text: 'Completed in 6 weeks',
                iconPath: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z',
            },
            {
                text: '20K+ posts',
                iconPath:
                    'M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3z',
            },
            {
                text: '95% user satisfaction',
                iconPath:
                    'M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zM14 9a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1v-6a1 1 0 00-1-1h-2z',
            },
        ],
    },
    {
        id: 4,
        key: 'dashboard',
        title: 'Analytics Dashboard',
        description: 'Interactive dashboard for real-time business analytics and reporting',
        imageLink: 'src/assets/image.jpg',
        tags: ['Angular', 'D3.js', 'Firebase'],
        overlayTitle: 'InsightHub',
        overlayDescription:
            'A powerful analytics tool providing real-time data visualization and customizable reports.',
        stats: [
            {
                text: 'Completed in 10 weeks',
                iconPath: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z',
            },
            {
                text: '30K+ data points',
                iconPath:
                    'M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3z',
            },
            {
                text: '99.9% data accuracy',
                iconPath:
                    'M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zM14 9a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1v-6a1 1 0 00-1-1h-2z',
            },
        ],
    },
];
